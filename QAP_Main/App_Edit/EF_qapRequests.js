...
var script_qapRequests = {
		...
		validate_ProjectID: function(sender) {
		  ...
		  this.validated_FK_QAP_Requests_ProjectID_main = true;
		  this.validate_FK_QAP_Requests_ProjectID(sender,Prefix);
		  },
		validate_SupplierID: function(sender) {
		  var Prefix = sender.id.replace('SupplierID','');
		  this.validated_FK_QAP_Requests_SupplierID_main = true;
		  this.validate_FK_QAP_Requests_SupplierID(sender,Prefix);
		  },
		validate_FK_QAP_Requests_ProjectID: function(o,Prefix) {
		  var value = o.id;
		  var ProjectID = $get(Prefix + 'ProjectID');
		  if(ProjectID.value==''){
		    if(this.validated_FK_QAP_Requests_ProjectID_main){
		  ...
		  value = value + ',' + ProjectID.value ;
		    ...
		    PageMethods.validate_FK_QAP_Requests_ProjectID(value, this.validated_FK_QAP_Requests_ProjectID);
		  },
		validated_FK_QAP_Requests_ProjectID_main: false,
		validated_FK_QAP_Requests_ProjectID: function(result) {
		  var p = result.split('|');
		  var o = $get(p[1]);
		  if(script_qapRequests.validated_FK_QAP_Requests_ProjectID_main){
		...
		validate_FK_QAP_Requests_SupplierID: function(o,Prefix) {
		  var value = o.id;
		  var SupplierID = $get(Prefix + 'SupplierID');
		  if(SupplierID.value==''){
		    if(this.validated_FK_QAP_Requests_SupplierID_main){
		  ...
		  value = value + ',' + SupplierID.value ;
		    ...
		    o.style.backgroundRepeat= 'no-repeat';
		    o.style.backgroundPosition = 'right';
		    PageMethods.validate_FK_QAP_Requests_SupplierID(value, this.validated_FK_QAP_Requests_SupplierID);
		  },
		validated_FK_QAP_Requests_SupplierID_main: false,
		validated_FK_QAP_Requests_SupplierID: function(result) {
		  var p = result.split('|');
		  var o = $get(p[1]);
		  if(script_qapRequests.validated_FK_QAP_Requests_SupplierID_main){
		    var o_d = $get(p[1]+'_Display');
		    try{o_d.innerHTML = p[2];}catch(ex){}
		  }
		  o.style.backgroundImage  = 'none';
