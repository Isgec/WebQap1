Partial Class GF_qapRequestsApprove
  Inherits SIS.SYS.GridBase
  Private _InfoUrl As String = "~/QAP_Main/App_Display/DF_qapRequestsApprove.aspx"
  ...
  End Sub
  Protected Sub GVqapRequestsApprove_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVqapRequestsApprove.RowCommand
		If e.CommandName.ToLower = "lgedit".ToLower Then
			Try
				Dim RequestNo As Int32 = GVqapRequestsApprove.DataKeys(e.CommandArgument).Values("RequestNo")  
				Dim RedirectUrl As String = TBLqapRequestsApprove.EditUrl & "?RequestNo=" & RequestNo
				Response.Redirect(RedirectUrl)
		...
		If e.CommandName.ToLower = "approvewf".ToLower Then
			Try
				Dim ApproverRemarks As String = CType(GVqapRequestsApprove.Rows(e.CommandArgument).FindControl("F_ApproverRemarks"), TextBox).Text
				Dim RequestNo As Int32 = GVqapRequestsApprove.DataKeys(e.CommandArgument).Values("RequestNo")
				SIS.QAP.qapRequestsApprove.ApproveWF(RequestNo, ApproverRemarks)
				GVqapRequestsApprove.DataBind()
			Catch ex As Exception
			End Try
		End If
		If e.CommandName.ToLower = "rejectwf".ToLower Then
			Try
				Dim ApproverRemarks As String = CType(GVqapRequestsApprove.Rows(e.CommandArgument).FindControl("F_ApproverRemarks"),TextBox).Text
				Dim RequestNo As Int32 = GVqapRequestsApprove.DataKeys(e.CommandArgument).Values("RequestNo")  
				SIS.QAP.qapRequestsApprove.RejectWF(RequestNo, ApproverRemarks)
				GVqapRequestsApprove.DataBind()
			Catch ex As Exception
			End Try
		End If
  End Sub
  Protected Sub GVqapRequestsApprove_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVqapRequestsApprove.Init
    DataClassName = "GqapRequestsApprove"
    SetGridView = GVqapRequestsApprove
  End Sub
